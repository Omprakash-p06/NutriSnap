...
def main():
    ...
    parser.add_argument(
        "--calibrator",
        default="checkpoints/efficientnet_mass_regressor_calibrator.joblib",
    )
    ...
    rgb_tensor = (
        transform_rgb(image=image)["image"].unsqueeze(0).to(device)
    )  # (1, 3, 224, 224)

    # Convert mask to [0, 1] range float32
    mask_norm = (mask_array > 0).astype(np.float32)
    mask_tensor = (
        transform_gray(image=mask_norm)["image"].unsqueeze(0).to(device)
    )  # (1, 1, 224, 224)
    ...
    depth_tensor = (
        transform_gray(image=depth_norm)["image"].unsqueeze(0).to(device)
    )  # (1, 1, 224, 224)

    # 5. Build 5-channel composite: RGB (3) + mask (1) + depth (1)
    composite = torch.cat(
        [rgb_tensor, mask_tensor, depth_tensor], dim=1
    )  # (1, 5, 224, 224)
    ...
    if mask_bool.sum() > 0:
        volume = (
            depth_tensor[mask_bool].mean().unsqueeze(0).unsqueeze(0).cpu()
        )  # (1, 1)
    else:

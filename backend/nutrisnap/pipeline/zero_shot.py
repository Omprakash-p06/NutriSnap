...
class ZeroShotFoodDetector:
    ...
    def _post_process(self, outputs, image, queries) -> list[dict]:
        ...
        if max_val > 0.01: # Only log if there's some signal
            logger.debug(f"Tile max score: {max_val:.4f} (threshold: {self.confidence_threshold})")
    ...
    def _detect_tiled(
        ...
        for y in y_starts:
            for x in x_starts:
                ...
                # Inference on tile
                inputs = self.processor(text=[queries], images=tile, return_tensors="pt").to(self.device)
                with torch.no_grad():
                    outputs = self.model(**inputs)

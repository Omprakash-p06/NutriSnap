...
class MultiFoodInferencePipeline:
    ...
    def __init__(
        ...
        logger.info(
            f"MultiFoodInferencePipeline initialized (device: {self.device_str}, llm: {enable_llm_validation})"
        )
    ...
    def predict(
        ...
        detections = self.detector.detect_food_only(
            image_rgb, max_detections=max_detections
        )
        ...
        normalized_boxes = MultiFoodDetector.normalize_boxes(
            boxes, (image_rgb.shape[1], image_rgb.shape[0])
        )
        ...
        merged_result = self.merger.merge_with_overlap_check(
            merged_detections, masks, depth_map
        )
        ...
        if self.enable_llm_validation and self.llm_validator is not None:
            ...
            try:
                ...
                validation_result = asyncio.run(
                    self.llm_validator.validate_meal(
                        items_json, merged_result.total_calories, None
                    )
                )
        ...
        logger.info(
            f"Pipeline complete: {result.item_count} items, {result.total_calories:.1f} kcal, {latency:.2f}s"
        )
...
def predict_multi(image: Union[str, Path], **kwargs) -> dict[str, Any]:

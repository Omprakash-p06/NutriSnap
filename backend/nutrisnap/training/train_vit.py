...
class CompositeDataset(Dataset):
    def __init__(self, ids, features_dir, dishes_df, is_train=False):
        ...
        # Spatial augmentations apply to all 5 channels to keep masks/depth aligned
        ...
        # Color augmentations apply only to the RGB channels
...
def train_one_epoch(
    ...
    for i, (x, v, y) in enumerate(pbar):
        ...
        if x.size(0) > 1:
            ...
            # Increase weight of correlation loss to 1.0 to strongly prioritize ranking
            loss = mse_loss + 1.0 * corr_loss
        else:
            loss = mse_loss
...
def main():
    ...
    # ---------------------------------------------------------
    # Freeze all ViT layers except the newly adapted 5-channel
    # patch embedding projection and the final regression head.
    logger.info("Freezing ViT backbone to prevent overfitting on small dataset...")
    for name, param in model.vit.named_parameters():
        if "embeddings.patch_embeddings.projection" not in name:

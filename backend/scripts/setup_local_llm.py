...
def _install_llama_cpp(backend: str, cuda_version: str = "cu124") -> bool:
    ...
    if backend == "cuda":
        wheel_index = _WHEEL_INDICES["cuda"].get(
            cuda_version, _WHEEL_INDICES["cuda"]["cu124"]
        )
        cmd = [
            sys.executable,
            "-m",
        ...
        if rc != 0:
            print(
                f"[ERROR] Compilation failed (exit {rc}). Check CMake and build tools."
            )
            return False
        return True
...
def _download_model(model_key: str) -> Path | None:
    ...
    try:
        ...
        print(f"[ERROR] Download failed: {e}")
        print(
            f"  Manual download URL: https://huggingface.co/{info['repo']}/resolve/main/{info['filename']}"
        )
...
def _print_next_steps(hw: dict, model_path: Path | None, backend: str) -> None:
    ...
    print(
        f"       --host 127.0.0.1 --port 8000 --n_gpu_layers {hw.get('n_gpu_layers', 0)}"
    )
    print()
    ...
    if hw.get("gpu_name"):
        print(
            f"GPU              : {hw['gpu_name']} ({hw.get('vram_gb', '?'):.1f} GB VRAM)"
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--detect-only", action="store_true", help="Only detect hardware, skip install"
    )
    parser.add_argument(
        "--no-download", action="store_true", help="Skip model download"
    )
    parser.add_argument(
        "--model-variant",

...
def batch_verify(upload_dir, mock=False, min_size=10240):
    logger.info(f"Batch verifying pipeline with images in: {upload_dir} (mock={mock}, min_size={min_size})")
    ...
    report_path = os.path.join(os.path.dirname(__file__), "..", "reports", "batch_verification_report.json")
    ...
    total_items = sum(r.get("items_detected", 0) for r in results if r["status"] == "success")
    logger.info(f"Summary: {success_count}/{len(valid_images)} images processed successfully.")
...
if __name__ == "__main__":
    ...
    upload_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "datasets", "uploads"))

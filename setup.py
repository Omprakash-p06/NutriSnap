...
def setup_env_files():
    ...
    if os.path.exists(backend_env_example):
        if not os.path.exists(backend_env):
            ...
            print("  ✓ Created. Please configure your API keys (e.g. GEMINI_API_KEY) in backend/.env if needed.")
        else:
...
def install_frontend_deps():
    ...
    success = run_command([npm_cmd if shutil.which(npm_cmd) else "npm", "install"], cwd="frontend")
    if success:
...
def download_ai_models():
    ...
    venv_dir = os.path.join("backend", "venv")
    is_windows = platform.system() == "Windows"
    python_exe = os.path.join(venv_dir, "Scripts" if is_windows else "bin", "python.exe" if is_windows else "python")
    download_script = os.path.join("backend", "scripts", "download_models.py")
    
    if not os.path.exists(download_script):
        print("✗ Model download script not found at backend/scripts/download_models.py.")
        return
        
    user_choice = input("Do you want to download the AI model weights (~1GB) now? (y/n) [y]: ").strip().lower()

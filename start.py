...
def get_backend_command():
    ...
    if not venv_dir:
        print(
            f"Error: Backend virtual environment not found in 'backend/' directory.",
            flush=True,
        )
        print(
            "Please run the setup script first to configure the environment: python setup.py",
            flush=True,
        )
        sys.exit(1)
...
def get_llama_server_command(model_path):
    ...
    return [
        python_exe,
        "-m",
        "nutrisnap.utils.local_llm_backend",
        "serve",
        "--model",
        model_path,
    ]
...
def main():
    ...
    if not os.path.exists(os.path.join("frontend", "node_modules")):
        print(
            "Error: Frontend 'node_modules' not found in 'frontend/' directory.",
            flush=True,
        )
        print(
            "Please run the setup script first to install dependencies: python setup.py",
            flush=True,
        )
        sys.exit(1)
    ...
    if gguf_model:
        ...
        print(
            "llama.cpp: No GGUF model found in backend/models/llm/. Chatbot will use cloud fallback.",
            flush=True,
        )
        print(
            "  Run: python backend/scripts/setup_local_llm.py --no-download", flush=True
        )

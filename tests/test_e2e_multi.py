...
class TestMultiFoodInferenceOrchestrator:
    ...
    def test_full_pipeline_latency_estimate(self):
        ...
        try:
            ...
            dummy_image = np.zeros((640, 640, 3), dtype=np.uint8)
            start = time.time()
            with patch.object(orchestrator, 'predict', return_value={"items": [], "total_calories": 0}):
                result = orchestrator.predict(dummy_image)
...
class TestPredictValidatedEndpoint:
    ...
    def test_endpoint_exists(self):
        ...
        assert "/validated" in content or "predict/validated" in content, \
    ...
    def test_endpoint_llm_reasoning_field(self):
        ...
        mock_response = {
            ...
            "total_calories": 0,
            "validation_summary": {
                "llm_reasoning": "All items are plausible"
            }
        }
...
class TestVRAMConstraints:
    ...
    def test_cpu_fallback_when_low_vram(self):
        ...
        try:
            from nutrisnap.pipeline.multi_food import MultiFoodDetector
            with patch('nutrisnap.pipeline.multi_food.get_available_vram', return_value=1.5):
                detector = MultiFoodDetector()

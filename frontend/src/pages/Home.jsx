...
export default function Home() {
  const { isAuthenticated, addXp, userSettings, currentUser, viewMode, setViewMode } = useAuth();
  ...
  const analyzePayload = async (type, payload) => {
    if (!navigator.onLine) {
      showNotification('Offline: Please connect to the internet to analyze meals.', 'offline');
      return;
    }
    ...
    try {
      ...
      if (err.message === 'AI_UNCERTAINTY') {
        showNotification("We couldn't identify that food clearly. Try typing it!", 'warning');
        setMode('search');
      } else {
          ...
          >
            {isAuthenticated && viewMode === 'app' ? (
              <section style={{ width: '100%', maxWidth: '900px', margin: '20px 0' }}>
                <InsightCards userEmail={isAuthenticated ? userSettings?.email || 'demo@demo.com' : null} />
                <div className="dashboard-layout-grid" style={{ display: 'flex', gap: '30px', flexWrap: 'wrap', justifyContent: 'center' }}>
                  <DashboardHeader 

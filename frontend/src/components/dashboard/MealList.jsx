...
export default function MealList({ todayMeals, deleteMeal }) {
  return (
    <div className="meal-list-container">
      <h3 style={{ margin: '0 0 15px 0', opacity: 0.8 }}>Today's Log ({todayMeals.length})</h3>
      
      {todayMeals.length === 0 ? (
        <p style={{ opacity: 0.6, fontStyle: 'italic' }}>Nothing logged yet! Scan a meal below to start.</p>
      ) : (
                  ...
                  {catMeals.map(meal => (
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                        <p style={{ margin: 0, fontWeight: '800', color: 'var(--primary-coral)' }}>{meal.calories} kcal</p>
                        <button onClick={()=>deleteMeal(meal.id)} className="delete-btn"><Trash2 size={16} /></button>
                      </div>
                    </motion.div>
                  ))}
                </motion.div>

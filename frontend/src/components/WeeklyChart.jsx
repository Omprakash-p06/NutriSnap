...
export default function WeeklyChart() {
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div style={tooltipStyle}>
          ...
          <p style={{ margin: "4px 0 0", color: "var(--primary-coral)" }}>
            {kcal} kcal
          </p>
          <p style={{ margin: "2px 0 0", fontSize: "0.75rem", opacity: 0.7 }}>
            {pct}% of goal
          </p>
        </div>
      );
  ...
  return (
    <div style={chartContainer}>
      <h3 style={{ margin: "0 0 16px 0", opacity: 0.85, fontSize: "1rem" }}>
        7-Day Calories
      </h3>
      <ResponsiveContainer width="100%" height={120}>
        <BarChart data={weekData} barSize={22}>
          ...
          <Tooltip
            ...
            cursor={{ fill: "rgba(255,107,90,0.06)" }}
          />
          <Bar dataKey="calories" radius={[6, 6, 0, 0]}>
            {weekData.map((entry, index) => (
              <Cell
                key={index}
                fill={entry.calories >= goal ? "#FF6B5A" : "url(#barGrad)"}
              />
            ))}
          </Bar>
          <defs>

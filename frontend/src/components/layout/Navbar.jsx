...
export default function Navbar() {
  ...
  const { isAuthenticated, currentUser, loginAsGuest, viewMode, setViewMode } = useAuth();
  ...
  return (
    ...
    >
      ...
      <div className="nav-links">
        {isMarketingMode ? (
          <>
            <a className="nav-link" style={{ color: 'var(--text-muted)' }}>Features</a>
            <a className="nav-link" style={{ color: 'var(--text-muted)' }}>How it Works</a>
            {isAuthenticated && (
              <a className="nav-link active" onClick={() => setViewMode('app')} style={{ color: '#FF6B5A', fontWeight: 800, cursor: 'pointer' }}>
                Back to Dashboard
              </a>
            )}
          </>
          ...
          <>
            <a className="nav-link" onClick={() => setViewMode('marketing')} style={{ cursor: 'pointer' }}>Showcase</a>
      ...
      <div className="nav-actions">
        ...
        {isAuthenticated ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
            ...
            <div className="clay-chip" style={{ minWidth: '160px', paddingRight: '12px' }}>
              <div style={{ 
                display: 'flex', 
                alignItems: 'center', 
                gap: '6px', 
                ...
                <span style={{ fontWeight: 800, fontSize: '0.85rem' }}>
                  <ShinyText text={currentUser?.level?.toString()} baseColor="#fff" speed={4} />
                </span>
              </div>
              
              <div style={{ display: 'flex', flexDirection: 'column', flex: 1, marginLeft: '2px' }}>
                <span style={{ fontSize: '0.75rem', fontWeight: 700, opacity: 0.85 }}>
                  {currentUser?.xp} <span style={{ opacity: 0.5, fontWeight: 500 }}>/ {getNextThreshold()}</span>
                </span>
              </div>

    ...
    >
      <SpotlightCard className="glass-card results-card">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-around', margin: '20px 0' }}>
          <div className="clay-badge" style={{ backgroundColor: '#FF6B5A', width: 'auto', padding: '0 20px', borderRadius: '25px', fontSize: '1.2rem' }}>
        ...
        <div style={{ padding: '0 10px', marginBottom: '25px' }}>
          <MacroBar label="Protein" value={result.protein * multiplier} max={100} color="#FF6B5A" />
          <MacroBar label="Carbs" value={result.carbs * multiplier} max={250} color="#FFB347" />
          <MacroBar label="Fat" value={result.fat * multiplier} max={100} color="#3ECFA0" />
        </div>
        
        <div style={{ width: "100%", height: 180, overflow: 'hidden', borderRadius: '12px', margin: '20px 0' }}>
          <ResponsiveContainer>
            <BarChart data={chartData}>
              <XAxis dataKey="name" stroke="var(--text)" />
              <YAxis stroke="var(--text)" />
              <Tooltip contentStyle={{ backgroundColor: 'var(--bg)', borderRadius: '8px' }} />
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', width: '100%', maxWidth: '400px', marginTop: '20px' }}>
          <select
            value={category}

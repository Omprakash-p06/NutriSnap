...
class NutritionPredictor:
    ...
    def __init__(
        ...
        logger_info = f"Loaded {len(self.models)} folds for ensembling on {device}"
        print(logger_info)
    ...
    def _load_ensemble(self, num_folds: int) -> List[nn.Module]:
        ...
        for i in range(num_folds):
            ...
            if not ckpt_path.exists():
                print(f"Warning: Fold {i} checkpoint not found at {ckpt_path}. Skipping.")
                continue
            ...
            model = NutritionRegressor(
                ...
                output_dims=self.model_cfg["model"]["output_dims"],

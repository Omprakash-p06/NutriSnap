...
class GeminiFallback:
    ...
    def __init__(self, config_path: str = "configs/api/config.yaml"):
        ...
        if self.enabled:
            ...
            self.model = genai.GenerativeModel(self.cfg.get("model", "gemini-1.5-flash"))
            
            with open(self.cfg["prompt_path"]) as f:
                self.prompt_template = f.read()
            logger.info(f"GeminiFallback initialized with model {self.cfg['model']}")
        else:
            logger.warning("GeminiFallback disabled (disabled in config or missing GEMINI_API_KEY)")

    async def refine(self, image_path: Path, initial_results: Dict[str, float]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Gemini successfully refined estimate: {refined.get('reasoning')}")
            return refined

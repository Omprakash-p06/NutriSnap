...
def run_sanity_checks(config_path):
    ...
    try:
        ...
        if preds.shape == (8, 4) and not torch.isnan(preds).any():
            ...
            logger.error(
                f"❌ Forward pass failed. Shape: {preds.shape}, NaN found: {torch.isnan(preds).any()}"
            )
            return
    except Exception as e:
    ...
    if last_loss < initial_loss * 0.1:
        logger.info(
            f"✅ Overfit check passed (Initial: {initial_loss:.4f} -> Final: {last_loss:.4f})"
        )
    else:
        logger.warning(
            f"⚠️ Overfit check inconclusive (Initial: {initial_loss:.4f} -> Final: {last_loss:.4f})"
        )
